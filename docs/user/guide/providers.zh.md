...
基础 profile 已使用 `http://127.0.0.1:20128/v1`、`openai-completions` 协议、安全的 `NINE_ROUTER_API_KEY` 凭据引用和入门模型 `trifecta` 注册 **9Router**。请在首次运行弹窗中粘贴端点密钥。如需使用其他模型或 combo，请打开**设置 → 模型 → 9Router → 编辑 → 自定义设置**，获取可用模型，然后应用 9Router 返回的准确 ID。
