...
The standalone composition below shows the complete explicit capability. A Profile based on `@truly-private/omdsh-base` keeps its existing Job rows, adds the product provider and tool rows, and does not mount duplicate Job services.
...
- id: tool-jobs
  name: '@truly-private/omdsh-tool-jobs'
