...
仅当 `DSH_CLIENT_BUILD_PROFILE` 为 `official` 时，本包才用 oh-my-deepseek-harness 标识填充 `sidebar.brand.mark`、`sidebar.brand.name` 和 `conversation.hero.brand.mark`。其他构建仍会加载插件，但不注册 occupant，因此显示 shell fallback。
