...
function declaration(
  ...
  return {
    name: short.startsWith('@') ? short : '@truly-private/omdsh-client-' + short,
...
describe('source package uses', () => {
  it('counts type imports, module augmentations, dynamic imports, and JSX', () => {
    const uses = collectSourcePackageUses('feature.tsx', [
      "import type { A } from '@truly-private/omdsh-a/subpath'",
      "declare module '@truly-private/omdsh-client-ui-slots' {}",
      "const load = () => import('@truly-private/omdsh-b')",
      'export const view = <div />',
      "export type { Local } from './local.ts'",
    ].join('\n'))

    expect([...uses].sort()).toEqual([
      '@truly-private/omdsh-a',
      '@truly-private/omdsh-b',
...
describe('package modes', () => {
  ...
  it('requires every preloaded external to have a parser preload row', () => {
    ...
    expect(collectClientPackageViolations(facts([], {
      ...
      + '"@truly-private/omdsh-client-runtime/client" has no matching PARSER_PRELOAD_IDS row in '
...
describe('dependency sections', () => {
  it('accepts dynamic peer plus dev relationships, static dev inputs, and private dependencies', () => {
    ...
    const runtime = pkg('runtime', {
      inject: ['@truly-private/omdsh-client-feature'],
      sourceUses: {
        '@truly-private/omdsh-agent': ['packages/client/runtime/src/index.ts'],
        '@truly-private/omdsh-client-ui-slots': ['packages/client/runtime/src/client/slots.ts'],
        react: ['packages/client/runtime/src/client/view.tsx'],
      },
      dependencies: { immer: '^10.1.1' },
      peerDependencies: {
        [CORDIS]: 'workspace:^',
        '@truly-private/omdsh-agent': 'workspace:^',
        '@truly-private/omdsh-client-feature': 'workspace:^',
      },
      devDependencies: {
        [CORDIS]: 'workspace:^',
        '@truly-private/omdsh-agent': 'workspace:^',
        '@truly-private/omdsh-client-feature': 'workspace:^',
        '@truly-private/omdsh-client-ui-slots': 'workspace:^',
        react: '^18.2.0',
      },
    })
  ...
  it('rejects internal dependencies, static peers, and mismatched peer development ranges', () => {
    ...
    const subject = pkg('feature', {
      sourceUses: {
        '@truly-private/omdsh-agent': ['packages/client/feature/src/index.ts'],
        [slots.name]: ['packages/client/feature/src/view.tsx'],
      },
      dependencies: { '@truly-private/omdsh-agent': 'workspace:^' },
  ...
  it('allows npm dependency cycles', () => {
    const a = pkg('a', {
      peerDependencies: { [CORDIS]: 'workspace:^', '@truly-private/omdsh-client-b': 'workspace:^' },
      devDependencies: { [CORDIS]: 'workspace:^', '@truly-private/omdsh-client-b': 'workspace:^' },
    })
    const b = pkg('b', {
      peerDependencies: { [CORDIS]: 'workspace:^', '@truly-private/omdsh-client-a': 'workspace:^' },
      devDependencies: { [CORDIS]: 'workspace:^', '@truly-private/omdsh-client-a': 'workspace:^' },
    })
...
describe('module requests', () => {
  it('accepts a dynamic row supplier and its client subpath', () => {
    const ui = declaration('ui', { external: ['@truly-private/omdsh-client-slots/client'] })
  ...
  it('rejects duplicates, empty values, self-requests, and missing suppliers', () => {
    const ui = declaration('ui', {
      external: ['', '@truly-private/omdsh-client-ui', '@truly-private/omdsh-missing', '@truly-private/omdsh-missing'],
      inject: ['', '@truly-private/omdsh-a', '@truly-private/omdsh-a'],
    })
  ...
  it('rejects synchronous module-request cycles but ignores inject cycles', () => {
    const a = declaration('a', {
      external: ['@truly-private/omdsh-client-b'],
      inject: ['@truly-private/omdsh-client-b'],
    })
    const b = declaration('b', {
      external: ['@truly-private/omdsh-client-a'],
      inject: ['@truly-private/omdsh-client-a'],
    })
...
describe('manifest declarations', () => {
  ...
  it('fixes unambiguous dependency sections and declaration entries', () => {
    ...
    roots.push(root)
    const subject = pkg('feature', {
      external: ['', 'react', '@truly-private/omdsh-client-feature', '@truly-private/omdsh-missing'],
      inject: ['', '@truly-private/omdsh-agent', '@truly-private/omdsh-agent'],
      sourceUses: {
        '@truly-private/omdsh-agent': ['packages/client/feature/src/index.ts'],
        '@truly-private/omdsh-client-ui-slots': ['packages/client/feature/src/view.tsx'],
      },
      dependencies: {
        [CORDIS]: 'workspace:^',
        '@truly-private/omdsh-agent': 'workspace:*',
    ...
    expect(fixed.dsh.client).toMatchObject({
      external: ['@truly-private/omdsh-missing'],
      inject: ['@truly-private/omdsh-agent'],
    })
